...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [runsData, territoriesData, rankData, notificationsData] = await Promise.all([
          getRuns(undefined, 5),
          getMyTerritories(),
          getUserGlobalRank(),
          getUnreadCount(),
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      >
        <div
          style={{ background: 'var(--color-bg-surface)', padding: '1.5rem', borderRadius: '8px' }}
        >
          <h3
            style={{
          ...
          <Link
            to="/leaderboards"
            style={{ color: 'var(--color-brand-primary)', fontSize: '0.85rem' }}
          >
            View Leaderboards →
          </Link>
        </div>
        ...
        >
          ...
          <Link to="/" style={{ color: 'var(--color-brand-primary)', fontSize: '0.85rem' }}>
            View Map →
          </Link>
        </div>
        ...
        >
          ...
          <Link to="/runs" style={{ color: 'var(--color-brand-primary)', fontSize: '0.85rem' }}>
            View History →
          </Link>
        </div>
        ...
        {runs.length === 0 ? (
          ...
          >
            <p style={{ color: 'var(--color-text-secondary)', marginBottom: '1rem' }}>
              You haven't claimed any ground yet.
            </p>
            <Link
              to="/"
              style={{
              ...
              >
                <div>
                  <div style={{ fontWeight: 'bold' }}>
                    {(run.distance_meters / 1000).toFixed(2)} km in{' '}
