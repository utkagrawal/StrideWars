...
import { MapContainer, TileLayer, Rectangle, Tooltip, useMapEvents, Polyline, useMap } from 'react-leaflet';
...
const TerritoryGrid = ({ currentUserId, refreshTrigger, onVisibleOwnersChange }: { currentUserId?: string, refreshTrigger: number, onVisibleOwnersChange: (owners: Record<string, {username: string, count: number}>) => void }) => {
  ...
  const cellsByOwner = useMemo(() => {
    const groups: Record<string, { username: string, capturedAt: string, hashes: string[], runIds: Set<string> }> = {};
...
export const TerritoriesMap = () => {
  ...
  const [stats, setStats] = useState<{ runs: number, areaSquareMeters: number, rank: number | null, unread: number, hasLoaded: boolean }>({ runs: 0, areaSquareMeters: 0, rank: null, unread: 0, hasLoaded: false });
  const [visibleOwners, setVisibleOwners] = useState<Record<string, {username: string, count: number}>>({});
  ...
  const currentGpsLog = [...runPoints].reverse().slice(0, 5).map(pt => ({ lat: pt.lat, lng: pt.lng, time: pt.recordedAt, acc: 0 }));
    ...
    if (runState === 'recording') {
      ...
      if (simulationMode) {
        simTimer = setInterval(() => {
          setRunPoints(prev => {
            const last = prev.length > 0 ? prev[prev.length - 1] : { lat: center[0], lng: center[1] };
        ...
        (pos) => {
          setRunPoints(prev => {
            ...
            if (prev.length > 0) {
              ...
              if (new Date(newPt.recordedAt).getTime() - new Date(lastPt.recordedAt).getTime() < 1000) {
                return prev;
              }
            }
  ...
  const handleGenerateRandomLoop = async () => {
    ...
    try {
      ...
      setRunState(prev => {
        ...
        addToast(`Random Loop generated! Claimed ${data.capturedTerritories.length} cells.`, 'success');
        return prev;
      });
    } catch (err: any) {
  ...
  if (runState === 'finished' && runPoints.length > 1) {
    ...
    if (dist > 30) {
      autoClosePositions = [[last.lat, last.lng], [first.lat, first.lng]];
    }
  }
  ...
  return (
    <div style={{ height: 'calc(100vh - 70px)', width: '100%', position: 'relative' }}>
      ...
      >
        ...
        />
        {hasCentered && <TerritoryGrid currentUserId={user?.id} refreshTrigger={refreshTrigger} onVisibleOwnersChange={setVisibleOwners} />}
        
        {polylinePositions.length > 0 && (
          <Polyline positions={polylinePositions} pathOptions={{ color: '#3b82f6', weight: 4 }} />
        )}
        {autoClosePositions.length > 0 && (
          <Polyline positions={autoClosePositions} pathOptions={{ color: '#3b82f6', weight: 4, dashArray: '5, 10' }} />
        )}
        {highlightLoopPoints.length > 0 && (
          <Polyline positions={highlightLoopPoints.map(p => [p.lat, p.lng] as [number, number])} pathOptions={{ color: '#eab308', weight: 5 }} />
        )}
      </MapContainer>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, color: 'var(--color-text-primary)' }}>
            {runState === 'idle' ? 'Ready to Run' : runState === 'recording' ? 'Recording...' : 'Run Finished'}
          </h3>
          {runState === 'idle' && (
        ...
        {(runState === 'recording' || runState === 'finished') && (
          <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--color-text-primary)', background: 'var(--color-bg-surface)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{formatTime(elapsedSeconds)}</div>
              <div style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Time Moving</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#3b82f6' }}>{(liveDistance / 1000).toFixed(2)}</div>
              <div style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Ground (km)</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{runPoints.length}</div>
              <div style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Closing Loop</div>
            </div>
          </div>
        )}
        ...
        {runState === 'recording' && currentGpsLog.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
            {currentGpsLog.map((gps, idx) => (
              <div key={idx} style={{ fontSize: '0.75rem', color: idx === 0 ? 'var(--color-text-primary)' : 'var(--color-text-secondary)', display: 'flex', justifyContent: 'space-between', background: 'var(--color-bg-surface)', padding: '0.5rem', borderRadius: '4px', opacity: 1 - (idx * 0.15) }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexDirection: 'column', position: 'relative' }}>
          {tourStep === 0 && (
            <div style={{ position: 'absolute', top: '-70px', left: '0', background: 'var(--color-brand-primary)', color: '#000', padding: '1rem', borderRadius: '8px', zIndex: 1100, boxShadow: '0 4px 12px rgba(0,0,0,0.5)', width: '100%' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '0.5rem', gap: '1rem' }}>
                <span onClick={dismissTour} style={{ fontSize: '0.8rem', cursor: 'pointer', textDecoration: 'underline' }}>Skip</span>
                <span onClick={advanceTour} style={{ fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer' }}>Next →</span>
              </div>
              <div style={{ position: 'absolute', bottom: '-10px', left: '50px', borderLeft: '10px solid transparent', borderRight: '10px solid transparent', borderTop: '10px solid var(--color-brand-primary)' }} />
            </div>
          )}

          {runState === 'idle' && (
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button onClick={handleStartRun} style={{ flex: 1, background: '#4ade80', color: '#000', fontSize: '1.1rem', padding: '0.75rem', fontWeight: 'bold', zIndex: tourStep === 0 ? 1200 : 'auto', position: 'relative' }}>
                Claim Ground
              </button>
            </div>
          )}
          ...
          {runState === 'recording' && (
            <>
              {simulationMode && (
                <button onClick={handleGenerateRandomPoint} style={{ background: 'var(--color-bg-surface)', border: '1px solid #3b82f6', color: '#3b82f6' }}>
                  Generate Random Point
                </button>
              )}
              
              <button onClick={handleFinishRun} style={{ background: '#ef4444', color: '#fff', fontSize: '1.1rem', padding: '0.75rem', fontWeight: 'bold' }}>
                Close Loop & Capture
              </button>
            </>
          )}
          ...
          {runState === 'finished' && (
            <>
              {submitting ? (
                ...
                <div style={{ background: 'rgba(74, 222, 128, 0.1)', padding: '1rem', borderRadius: 'var(--radius-md)', border: '1px solid rgba(74, 222, 128, 0.3)' }}>
                  <h4 style={{ color: '#4ade80', margin: '0 0 0.5rem 0', fontSize: '1.3rem' }}>🎉 You claimed this ground!</h4>
                  ...
                  {captureSummary.capturedTerritories.length > 0 && (
                    <div style={{ marginTop: '0.5rem' }}>
                      <span style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>From opponents:</span>
                      <ul style={{ maxHeight: '80px', overflowY: 'auto', paddingLeft: '1rem', fontSize: '0.85rem', margin: '0.25rem 0' }}>
                        {captureSummary.capturedTerritories.map((t: any) => (
                          <li key={t.geohash}>
                            {t.geohash} {t.previousOwnerId && <span style={{ color: '#ef4444', marginLeft: '0.5rem' }}>(Taken)</span>}
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <Link to="/leaderboards" style={{ display: 'block', marginTop: '1rem', background: '#4ade80', color: '#000', padding: '0.75rem', borderRadius: 'var(--radius-sm)', textDecoration: 'none', fontWeight: 'bold', textAlign: 'center' }}>
                    View your new Rank →
                  </Link>
                </div>
              ) : (
                <div style={{ textAlign: 'center', padding: '1rem', color: 'var(--color-text-secondary)', background: 'var(--color-bg-surface)', borderRadius: 'var(--radius-md)' }}>
                  Run logged, but no territory was captured.
                </div>
              )}
              <button onClick={handleReset} style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }}>
                Close
              </button>
            </>
          )}
          ...
          <button onClick={handleGenerateRandomLoop} disabled={submitting} style={{ background: 'var(--color-bg-surface)', border: '1px solid #3b82f6', color: '#3b82f6', fontSize: '0.9rem', padding: '0.75rem', fontWeight: 'bold', width: '100%', marginTop: '0.5rem' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div 
            style={{ padding: '0.75rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', background: 'var(--color-bg-surface)' }}
          ...
          {hudExpanded && (
            <div style={{ padding: '1rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Rank</div>
                <Link to="/leaderboards" style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--color-brand-primary)', textDecoration: 'none' }}>
                  {stats.rank ? `#${stats.rank}` : '-'}
                </Link>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Area Claimed</div>
                <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>{formatArea(stats.areaSquareMeters)}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Recent Runs</div>
                <Link to="/dashboard" style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--color-brand-primary)', textDecoration: 'none' }}>
                  Dashboard →
                </Link>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Alerts</div>
                <Link to="/notifications" style={{ fontSize: '1.25rem', fontWeight: 'bold', color: stats.unread > 0 ? '#ef4444' : 'var(--color-text-primary)', textDecoration: 'none' }}>
                  {stats.unread}
                </Link>
              </div>
            </div>
          ...
          {tourStep === 2 && (
            <div style={{ position: 'absolute', top: '50px', right: '270px', background: 'var(--color-brand-primary)', color: '#000', padding: '1rem', borderRadius: '8px', zIndex: 1100, boxShadow: '0 4px 12px rgba(0,0,0,0.5)', width: '250px' }}>
              Watch your rank climb as you capture more cells.
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '0.5rem' }}>
                <span onClick={dismissTour} style={{ fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer' }}>Got it</span>
              </div>
              <div style={{ position: 'absolute', top: '10px', right: '-10px', borderTop: '10px solid transparent', borderBottom: '10px solid transparent', borderLeft: '10px solid var(--color-brand-primary)' }} />
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '0.9rem', color: 'var(--color-text-secondary)' }}>Visible Map Owners</h4>
            ...
            {user && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem' }}>
                ...
                {visibleOwners[user.id] && (
                  <span style={{ marginLeft: 'auto', color: 'var(--color-text-secondary)' }}>{visibleOwners[user.id].count}</span>
                )}
              </div>
            )}
            
            {/* Show top other users */}
            {topOwners.filter(([id]) => id !== user?.id).map(([ownerId, info]) => (
              <div key={ownerId} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem' }}>
                <div style={{ width: '12px', height: '12px', borderRadius: '50%', background: colorForUser(ownerId, user?.id) }} />
                <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{info.username}</span>
                <span style={{ marginLeft: 'auto', color: 'var(--color-text-secondary)' }}>{info.count}</span>
              </div>
            ))}
          ...
          {tourStep === 1 && (
            <div style={{ position: 'absolute', top: '100px', right: '270px', background: 'var(--color-brand-primary)', color: '#000', padding: '1rem', borderRadius: '8px', zIndex: 1100, boxShadow: '0 4px 12px rgba(0,0,0,0.5)', width: '250px' }}>
      ...
      {stats.hasLoaded && stats.areaSquareMeters === 0 && runState === 'idle' && (
        <div style={{
          ...
          <p style={{ color: 'var(--color-text-secondary)', margin: 0, fontSize: '1.2rem' }}>Run a loop to make your first move.</p>
        </div>
      )}
    </div>
  );
