...
describe('Leaderboards Integration', () => {
  ...
  it('updates the leaderboard when a user captures territories via run upload', async () => {
    const pt = { lat: 37.0, lng: -122.0, ele: 10, recordedAt: '2023-01-01T10:00:01Z' };
    
    // Upload run for u1
    const runRes = await request(app)
      .post('/api/runs')
      .set('Authorization', `Bearer ${u1Token}`)
      .send({ clientRunId: require('crypto').randomUUID(), startedAt: '2023-01-01T10:00:00Z', points: [pt] });
    ...
    expect(res.body.entries[0].territoryCount).toBe(1);
    ...
    expect(meRes.body.territoryCount).toBe(1);
  });
});
