...
async function computeUserRegionalTerritoryAreas(userId: string): Promise<Map<string, number>> {
  const { rows } = await pool.query('SELECT geohash FROM territories WHERE owner_id = $1', [
    userId,
  ]);
...
async function hydrateUsernames(
  ...
  const { rows } = await pool.query(`SELECT id, username FROM users WHERE id = ANY($1)`, [userIds]);
...
export async function getUserGlobalRank(
  userId: string
): Promise<{ rank: number | null; areaSquareMeters: number }> {
  try {
