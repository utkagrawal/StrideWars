...
router.post(
  ...
  [
    ...
    body('points').isArray({ min: 1, max: 20000 }).withMessage('points must be an array of length 1 to 20,000'),
    body('points.*.lat')
    ...
    body('points.*.recordedAt')
      .isISO8601()
      .withMessage('Point recordedAt must be an ISO8601 date'),
  ],
  validate,
...
router.get(
  ...
  [
    ...
    query('tolerance').optional().isFloat({ min: 0 }).withMessage('tolerance must be a positive number'),
  ],
  validate,
